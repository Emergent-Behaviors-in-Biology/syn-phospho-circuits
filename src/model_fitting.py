...
#                 bounds.append((0.9, 1.1))
        ...
        for exp_name, row in df_dataset_key.iterrows():
            model = row['model']
            if model == 'two_layer' or model == 'two_layer_nowriter' or model == 'two_layer_noeraser' or model == 'two_layer_mono':  
...
#                         bounds.append((0.9, 1.1))
    ...
    for exp_name, row in df_dataset_key.iterrows():
        ...
        if model == 'two_layer_mono':
            
                # background phospho rate for second kinase/ first substrate
                if 'mono_kin2_bg_phospho_rate' not in prefit_params and 'mono_kin2_bg_phospho_rate' not in param_to_index:
                    param_to_index['mono_kin2_bg_phospho_rate'] = len(x0)
                    x0.append(-2.0)
                    bounds.append((-8, 1))

                dataset_to_params[exp_name].append('mono_kin2_bg_phospho_rate')
                
                # background phospho rate for second substrate
                if 'mono_sub2_bg_phospho_rate' not in prefit_params and 'mono_sub2_bg_phospho_rate' not in param_to_index:
                    param_to_index['mono_sub2_bg_phospho_rate'] = len(x0)
                    x0.append(-2.0)
                    bounds.append((-8, 1))
        ...
        # kinase parameters  
        if model == 'push' or model == 'pushpull' or model == 'two_layer' or model == 'two_layer_noeraser' or model == 'two_layer_mono':
        ...
        # pptase parameters
        if model == 'pushpull' or model == 'two_layer' or model == 'two_layer_nowriter' or model == 'two_layer_mono':
...
def predict(x, args, df_copy, n_jobs=1):
...
#         print(exp_name, len(df_tmp.index))
        ...
        if row['model'] == 'substrate_only':
        ...
        elif row['model'] == 'two_layer' or row['model'] == 'two_layer_mono':
            
            kinase2_phospho_MOCU_predict, phospho_MOCU_predict = thermo.predict_twolayer(df_tmp['kinase_MOCU_infer'].values, df_tmp['pptase_MOCU_infer'].values, df_tmp['kinase2_MOCU_infer'].values, df_tmp['substrate_MOCU_infer'].values, *params, n_jobs=n_jobs)
            ...
            kinase2_phospho_MOCU_predict, phospho_MOCU_predict = thermo.predict_twolayer_nowriter(df_tmp['pptase_MOCU_infer'].values, df_tmp['kinase2_MOCU_infer'].values, df_tmp['substrate_MOCU_infer'].values, *params, n_jobs=n_jobs)
            df_copy.loc[df_tmp.index, 'phospho_MOCU_predict'] = phospho_MOCU_predict
            df_copy.loc[df_tmp.index, 'kinase2_phospho_MOCU_predict'] = kinase2_phospho_MOCU_predict
            
        elif row['model'] == 'two_layer_noeraser':
            
            kinase2_phospho_MOCU_predict, phospho_MOCU_predict = thermo.predict_twolayer_noeraser(df_tmp['kinase_MOCU_infer'].values, df_tmp['kinase2_MOCU_infer'].values, df_tmp['substrate_MOCU_infer'].values, *params, n_jobs=n_jobs)
            df_copy.loc[df_tmp.index, 'phospho_MOCU_predict'] = phospho_MOCU_predict
            df_copy.loc[df_tmp.index, 'kinase2_phospho_MOCU_predict'] = kinase2_phospho_MOCU_predict
           
     
    
def loss_lognormal_bg(x, args, n_jobs=1):
    ...
    predict(x, args, df_copy, n_jobs=n_jobs)
    ...
    for exp_name, row in df_dataset_key.iterrows():
        ...
        if 'substrate_phospho_factor' in row.index.values:
            substrate_phospho_factor = row['substrate_phospho_factor']
        else:
            substrate_phospho_factor = 1.0
        ...
        if len(df_exp.index) > 0:
            loss += loglikelihood_lognormal_bg(df_exp['phospho_GFP_infer'].values, substrate_phospho_factor*df_exp['phospho_MOCU_predict'].values, noise_models['phospho']['GFP2MOCU'], params[0])
...
def fit(df_dataset_key, df_data, df_params=None, noise_models=None, n_jobs=1):
    
    print("cpus:", joblib.cpu_count())
...
#         print("Loss:", loss(x, args))
    ...
    res = opt.minimize(loss_lognormal_bg, x0, args=(args, n_jobs), method='L-BFGS-B', 
...
def calc_error(df_dataset_key, df_data, df_params, tol=1e-4, noise_models=None, n_jobs=1):
    ...
    f0 = loss_lognormal_bg(x0, args, n_jobs=n_jobs)
    ...
    for p in param_to_index:
        ...
        def func(val):
            ...
            f = loss_lognormal_bg(x, args, n_jobs=n_jobs) - (1+tol)*f0
